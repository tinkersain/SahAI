...
- जो जानकारी "✅ उपयोगकर्ता की जानकारी" में दी गई है, वो दोबारा कभी न पूछें!
- अगर उम्र पहले से है तो उम्र मत पूछें
- अगर आय पहले से है तो आय मत पूछें  
- सिर्फ वही जानकारी पूछें जो अभी तक नहीं मिली है
...
- हमेशा महिला की भाषा शैली में बोलें (feminine verbs)"""
    ...
    def _build_user_context(self, user_data: Dict[str, Any], conversation_history: str) -> str:
        ...
        if user_data:
            # Format user data clearly for the LLM
            collected = []
            if user_data.get('age'):
                collected.append(f"उम्र: {user_data['age']} वर्ष")
            if user_data.get('income'):
                collected.append(f"वार्षिक आय: ₹{user_data['income']}")
            if user_data.get('gender'):
                collected.append(f"लिंग: {user_data['gender']}")
            if user_data.get('category'):
                collected.append(f"श्रेणी: {user_data['category']}")
            if user_data.get('bpl'):
                collected.append("BPL: हां")
            
            if collected:
                context_parts.append(f"✅ उपयोगकर्ता की जानकारी (पहले से मिली, दोबारा मत पूछें):\n" + ", ".join(collected))
                context_parts.append("⚠️ महत्वपूर्ण: ऊपर दी गई जानकारी दोबारा न पूछें!")
